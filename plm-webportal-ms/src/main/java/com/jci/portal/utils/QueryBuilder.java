...
public class QueryBuilder {
	...
	public static String plmQuery(DataHelper request){
		LOG.info("#### Starting QueryBuilder.plmQuery ###" + request);
     ...
     //  String timestampFilter = TableQuery.generateFilterCondition(TIMESTAMP, QueryComparisons.LESS_THAN_OR_EQUAL,request.getTimestamp());
       ...
       LOG.info("#### Ending  QueryBuilder.plmQuery ###" + combinedFilter);
	...
	public static String processPosQuery(String partitionKey, List<String> plmList){
		LOG.info("#### Starting QueryBuilder.processPosQuery ###" + plmList);
	   ...
	   for(int i=0;i<plmList.size();i++){  
		 builder.append(" ( ");
		 rowKeyFilter = TableQuery.generateFilterCondition(ROWKEY, QueryComparisons.EQUAL, plmList.get(i));
