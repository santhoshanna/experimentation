    ...
    function init(){
        ...
        serviceObj.pullPoData(reqObj).then(function( data, textStatus, jqXHR ) {
            if(!data.error){
				...
				/*$('.nav-second-level li.disabled a').off('click').on('click',function(e){
					e.preventDefault();
					e.stopPropagation();
    ...
    */
	...
	function buildECNGraph(graphData,erpData,container){
		var graphDataObject={},categoryArr=[], categoryData={},errorGraphData=[], processGraphData=[];
			...
			var name = {"x-axis":"","y-axis":"Purchase Orders","title":"Windchill PLM Dashboard"};
			if(container)
				commonUtil.plotGraph(plotData,name,categoryArr,container);
			else
				commonUtil.plotGraph(plotData,name,categoryArr);
	...
	$(document).on('click','.plm .autopager', function(e) {
		  ...
		  if(mainTab == "ecn"){
			if($('section.active').attr('data-item-type') == "error")
				url = "/getSegmentedPlmDetails";          // error ecn url
			else
				url = "/getSegmentedErrorDetails";        //ecn url 
		  }  
	...
	$(document).on('click','#submitECRErrBtn', function(e) {
		var poNumArray = [];
		var poUiprocessedArray=[];
		var poUiprocessedbyArray=[];
		var pouiprocessdateArray=[];
		
	//	var activeGrid = $('#main2 .tab-pane.fade.in.active .grid-style').attr('id');
		var activeGrid = ($('.nav.nav-second-level.collapse.in a.active').attr('grid')).replace('ecn','ecnerror');
		
		var selectedRows = mainGrid[activeGrid].grid.getSelectedRows();
		 var div=$('.tableError').html();
		if(selectedRows.length == 0){
			toastr.error('Select PLM to be processed');
			return;
		}
		...
		for(var i = 0 ; i < selectedRows.length ;i++){
			...
			poUiprocessedArray.push(item.Uiprocessed);
			poUiprocessedbyArray.push(item.Uiprocessedby);
			pouiprocessdateArray.push(item.Uiprocessdate);
		}
		...
		var userName = $('#username').text();
		var globalId = $('#username').attr('data-global-id');
		var sendObj = JSON.stringify({"erpName":erp,"ecnNumber":poNumArray,"globalId": globalId,"userName": userName,"comment":comment,"uiprocessed":poUiprocessedArray,"uiprocessedby":poUiprocessedbyArray,"uiprocessdate":pouiprocessdateArray});
		commonUtil.addLoader();
		
		
		//...........insert controllers url.......
		
		
		serviceObj.callToSever("/processErrorPos",sendObj,"POST").then(function(result,status,xhr){
			
			commonUtil.removeLoader();  
				console.log(result);  
				success:  $('.tableError').html(div),
				$('input:checkbox').removeAttr('checked');
				 // $(".txtXML").load("/processErrorPos");
